...
namespace GraphQLDemo.Services
{
    public class BlogService
    {
        ...
        private readonly IEnumerable<Author> authors;

        public BlogService(IAuthorService authorService)
        {
            _authorService = authorService;
            authors = _authorService.GetAuthors();
            var dinoEsposito = authors.Where(a => a.Id == 1).FirstOrDefault();
            var lanceTalbert = authors.Where(a => a.Id == 2).FirstOrDefault();
